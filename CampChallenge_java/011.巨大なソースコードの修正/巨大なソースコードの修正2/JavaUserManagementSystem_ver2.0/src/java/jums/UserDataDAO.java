...
public class UserDataDAO {
    ...
    // search の結果が一人分しか出なかったので、ArrayListに保存する処理に変更　bym
    ...
    public ArrayList<UserDataDTO> search(UserDataDTO ud) throws SQLException{
        ...
        try{
            ...
            while(rs.next()){
                ...
                UserDataDTO rs1 = new UserDataDTO();
                rs1.setUserID(rs.getInt(1));
                rs1.setName(rs.getString(2));
                rs1.setBirthday(rs.getDate(3));
                rs1.setTell(rs.getString(4));
                rs1.setType(rs.getInt(5));
                rs1.setComment(rs.getString(6));
                rs1.setNewDate(rs.getTimestamp(7));
...
//以下追加 bym
    /**
     * データの上書き処理を行う。現在時刻は挿入直前に生成(update日時に書き換えられる。)
     * @param ud 対応したデータを保持しているJavaBeans
     * @throws SQLException 呼び出し元にcatchさせるためにスロー 
     */
